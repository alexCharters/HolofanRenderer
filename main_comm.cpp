...
const char* tex_fragment_shader =
    ...
    "void main () {"
        ...
        "theta = gl_FragCoord.y * (2.0*3.1415926535897932384626433832795/float(1024));"
        "x = r * cos(theta);"
        "y = r * sin(theta);"
...
static void drawShaderWithVertexBufferObject()
{
    ...
    glViewport(0,0,numLEDs,radialRes);
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
    ...
    char   *pixels = (char*)malloc(numLEDs*radialRes*3);
...
{
    glutInit(&argc, argv);
    glutInitWindowSize(numLEDs,radialRes);
